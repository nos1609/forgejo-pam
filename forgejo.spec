...
Source0:        %{gosource}
# The following two are generated by setting "--with bundle_vendored" as an rpmbuild flag.
...
GO_BUNDLE_TARBALL="%{S:1}"
NODEJS_BUNDLE_TARBALL="%{S:2}"

%if %{without bundle_vendored_force}
if [ -e "$GO_BUNDLE_TARBALL" ]; then
    echo "Skipping overwriting $GO_BUNDLE_TARBALL"
else
%endif
...
if [ -e "$NODEJS_BUNDLE_TARBALL" ]; then
    echo "Skipping overwriting $NODEJS_BUNDLE_TARBALL"
else
%endif
    ...
    cd "%{_builddir}"
    tar -cvaf "$NODEJS_BUNDLE_TARBALL" "%{buildsubdir}/node_modules"
    popd
%if %{without bundle_vendored_force}
fi
%endif
